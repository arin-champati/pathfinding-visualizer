...
def speed(node_1, node_2, width):
    ...
    # break ties as well
    return (distance / 60) / ((1/60) / (width * width))
...
def a_star(draw_path, grid, start_node, end_node, width, heuristic):
    index = 0
    if heuristic == "speed":
        function1 = partial(speed, start_node, end_node, width)
    ...
    while not open_list.empty():
        ...
        for neighbor in current.neighbors:
            if heuristic is "speed":
                temp_g_score = current.g + current.cost() # cost is time
            ...
            if temp_g_score < neighbor.g:
                if heuristic == "speed":
                    function2 = partial(speed, neighbor, end_node, width)
                elif heuristic == "distance":
